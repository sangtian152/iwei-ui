<template>
  <div class="page-swipe">
    <h1 class="page-title">Swipe</h1>
    <p class="page-swipe-desc">基础用法</p>
    <zmbl-swipe :auto="4000">
      <zmbl-swipe-item class="slide1">1</zmbl-swipe-item>
      <zmbl-swipe-item class="slide2">2</zmbl-swipe-item>
      <zmbl-swipe-item class="slide3">3</zmbl-swipe-item>
    </zmbl-swipe>

    <p class="page-swipe-desc">隐藏 indicators</p>
    <zmbl-swipe :show-indicators="false">
      <zmbl-swipe-item class="slide1">1</zmbl-swipe-item>
      <zmbl-swipe-item class="slide2">2</zmbl-swipe-item>
      <zmbl-swipe-item class="slide3">3</zmbl-swipe-item>
    </zmbl-swipe>

    <p class="page-swipe-desc">取消自动播放</p>
    <zmbl-swipe :auto="0">
      <zmbl-swipe-item class="slide1">1</zmbl-swipe-item>
      <zmbl-swipe-item class="slide2">2</zmbl-swipe-item>
      <zmbl-swipe-item class="slide3">3</zmbl-swipe-item>
    </zmbl-swipe>

    <p class="page-swipe-desc">设置默认显示页</p>
    <zmbl-swipe :auto="0" :defaultIndex="1">
      <zmbl-swipe-item class="slide1">1</zmbl-swipe-item>
      <zmbl-swipe-item class="slide2">2</zmbl-swipe-item>
      <zmbl-swipe-item class="slide3">3</zmbl-swipe-item>
    </zmbl-swipe>

    <p class="page-swipe-desc">单个幻灯片</p>
    <zmbl-swipe :show-indicators="false">
      <zmbl-swipe-item class="slide1">SINGLE SLIDE</zmbl-swipe-item>
    </zmbl-swipe>
  </div>
</template>

<style lang="scss">
  .page {
    &-swipe {
      &-desc {
        text-align: center;
        color: #666;
        margin-bottom: 5px;
      }
      .zmbl-swipe {
        height: 200px;
        color: #fff;
        font-size: 30px;
        text-align: center;
        margin-bottom: 20px;
      }

      .zmbl-swipe-item {
        line-height: 200px;
      }

      .slide1 {
        background-color: #0089dc;
        color: #fff;
      }

      .slide2 {
        background-color: #ffd705;
        color: #000;
      }

      .slide3 {
        background-color: #ff2d4b;
        color: #fff;
      }
    }
  }
</style>
