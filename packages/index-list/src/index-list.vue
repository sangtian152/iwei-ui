<template>
  <div class="zmbl-indexlist">
    <ul class="zmbl-indexlist-content" ref="content" :style="{ 'height': currentHeight + 'px', 'margin-right': navWidth + 'px'}">
      <slot></slot>
    </ul>
    
    <div class="zmbl-indexlist-nav" @touchstart="handleTouchStart" ref="nav">
      <ul class="zmbl-indexlist-navlist">
        <li class="zmbl-indexlist-navitem" v-for="section in sections" :key="section.index">{{ section.index }}</li>
      </ul>
    </div>
    
    <div class="zmbl-indexlist-indicator" v-if="showIndicator" v-show="moving">{{ currentIndicator }}</div>
  </div>
</template>
