<template>
  <div class="zmbl-palette-button" :class="{ expand: expanded, 'zmbl-palette-button-active': transforming }"
    @animationend="onMainAnimationEnd" @webkitAnimationEnd="onMainAnimationEnd" @mozAnimationEnd="onMainAnimationEnd">
    <div class="zmbl-sub-button-container">
      <slot></slot>
    </div>
    <div @touchstart="toggle" class="zmbl-main-button" :style="mainButtonStyle">
      {{content}}
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    props: {
      ...
      mainButtonStyle: {
        type: String,           // 应用到 zmbl-main-button 上的 class
        default: ''
      }
    },
    methods: {
...
<style lang="scss">
  ...
  .zmbl-palette-button-active{
    animation: zmbl-zoom 0.5s ease-in-out;
  }
  .zmbl-sub-button-container>*{
    position:absolute;
    top:15px;
    left:15px;
    width:25px;
